...
const TarjetaEstadistica = ({ config, valor, spring }) => {
  const estiloTarjeta = {
    background: config.titulo === "Clientes Registrados" 
      ? "white" 
    ...
    padding: "2rem",
    boxShadow: config.titulo === "Clientes Registrados" 
      ? "0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06)"
      : "0 8px 32px rgba(0,0,0,0.1)",
    border: config.titulo === "Clientes Registrados" 
      ? "1px solid #e2e8f0" 
  ...
  const manejarHover = (e, isEnter) => {
    if (isEnter) {
      e.target.style.transform = "translateY(-5px)";
      e.target.style.boxShadow = config.titulo === "Clientes Registrados"
        ? "0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04)"
        : "0 12px 40px rgba(0,0,0,0.15)";
    } else {
      e.target.style.transform = "translateY(0)";
      e.target.style.boxShadow = estiloTarjeta.boxShadow;
    }
  };
  ...
  return (
    <div 
      style={estiloTarjeta}
      onMouseEnter={(e) => manejarHover(e, true)}
      onMouseLeave={(e) => manejarHover(e, false)}
    >
      <div style={{
        display: "flex",
  ...
  useEffect(() => {
    const fetchDatos = async () => {
      try {
        ...
        setDatos(resultados.map((res, index) => {
